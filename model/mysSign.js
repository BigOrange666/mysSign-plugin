...
export default class MysSign extends base {
    ...
    static async sign(e) {
        ...
        /** 获取个人ck */
        let ck = await gsCfg.getBingCkSingle(e)
    ...
    async getValidate(sign) {
        ...
        for (let i = 0; i < 2; i++) {
            ...
            if (validate.data?.validate) {
                data['headers']['x-rpc-challenge'] = validate.data.challenge
                data['headers']['x-rpc-validate'] = validate.data.validate
                data['headers']['x-rpc-seccode'] = `${validate.data.validate}|jordan`
